...
namespace NurbsSharp.Intersection
{
    ...
    public static class RaySurfaceIntersector
    {
        ...
        /// Uses Fast method to find initial seed point, then refines with Newton-Raphson.
        /// </summary>
        /// <param name="ray">Ray to test</param>
        /// <param name="surface">Surface to test against</param>
        ...
        /// <param name="tessellationResolution">Tessellation resolution for initial Fast intersection (default: 20)</param>
        /// <returns>True if an intersection was found</returns>
        public static bool IntersectsRobust(Ray ray, NurbsSurface surface, out RaySurfaceIntersection intersection, double tolerance = 1e-6, int maxIterations = 50, int tessellationResolution = 20)
        {
            intersection = default;
            Guard.ThrowIfNull(surface, nameof(surface));
            ...
            // Use Fast method to get initial seed point
            if (!IntersectsFast(ray, surface, out var fastHit, numPointsU: tessellationResolution, numPointsV: tessellationResolution))
                return false;
            ...
            // Use Fast hit as initial guess for Newton-Raphson
            double u0 = fastHit.U;
            double v0 = fastHit.V;
            double t0 = fastHit.T;
            ...
            if (NewtonRaphsonRaySurface(ray, surface, u0, v0, t0, tolerance, maxIterations, out double u, out double v, out double t))
            {
                ...
                if (u >= uMin && u <= uMax && v >= vMin && v <= vMax && t > 0)
                {
                    ...
                    {
                        intersection = new RaySurfaceIntersection(t, u, v, point, -1);
                        return true;
                    }
                }
            }
        ...
        /// High-precision method suitable for CAD/CAM and accurate geometric computations.
        /// Uses Fast method to find initial seed points, then refines with Newton-Raphson.
        /// </summary>
        public static List<RaySurfaceIntersection> IntersectAllRobust(Ray ray, NurbsSurface surface, double tolerance = 1e-6, int maxIterations = 50, int tessellationResolution = 30)
        {
            var result = new List<RaySurfaceIntersection>();
            Guard.ThrowIfNull(surface, nameof(surface));
            ...
            // Use Fast method to get initial seed points
            var fastHits = IntersectAllFast(ray, surface, numPointsU: tessellationResolution, numPointsV: tessellationResolution);
            if (fastHits.Count == 0)
                return result;
            ...
            // Refine each Fast hit using Newton-Raphson
            foreach (var fastHit in fastHits)
            {
                double u0 = fastHit.U;
                double v0 = fastHit.V;
                double t0 = fastHit.T;
        ...
        private static bool NewtonRaphsonRaySurface(Ray ray, NurbsSurface surface, double u0, double v0, double t0, double tolerance, int maxIterations, out double u, out double v, out double t)
        {
            ...
            for (int iter = 0; iter < maxIterations; iter++)
            {
                ...
                J[0] = new double[] { Su.X, Sv.X, -ray.Direction.X };
                J[1] = new double[] { Su.Y, Sv.Y, -ray.Direction.Y };
                J[2] = new double[] { Su.Z, Sv.Z, -ray.Direction.Z };
