...
{
    [TestFixture]
    public class DegreeOperatorTest
    {
        ...
        [Test]
        public void ElevateDegree_Surface_UDirection_BasicTest()
        {
            ...
            var elevatedSurface = DegreeOperator.ElevateDegree(surface, 1, 0);

            // Verify dimensions
            using (Assert.EnterMultipleScope())
            {
                Assert.That(elevatedSurface.DegreeU, Is.EqualTo(degreeU + 1));
                Assert.That(elevatedSurface.DegreeV, Is.EqualTo(degreeV));
                Assert.That(elevatedSurface.ControlPoints, Is.Not.Empty);
            }

            // Verify shape is preserved by sampling points
            double[] samplePoints = [0, 0.1, 0.25, 0.5, 0.75, 0.9, 1];
            foreach (var u in samplePoints)
            {
                foreach (var v in samplePoints)
                {
                    var originalPos = surface.GetPos(u, v);
                    var elevatedPos = elevatedSurface.GetPos(u, v);

                    using (Assert.EnterMultipleScope())
                    {
                        Assert.That(elevatedPos.X, Is.EqualTo(originalPos.X).Within(1e-6));
                        Assert.That(elevatedPos.Y, Is.EqualTo(originalPos.Y).Within(1e-6));
                        Assert.That(elevatedPos.Z, Is.EqualTo(originalPos.Z).Within(1e-6));
                    }
                }
            }
        }
        ...
        [Test]
        public void ElevateDegree_Surface_VDirection_BasicTest()
        {
            ...
            // Act: Elevate degree in V direction
            var elevatedSurface = DegreeOperator.ElevateDegree(surface, 0, 1);

            // Assert: Verify dimensions
            using (Assert.EnterMultipleScope())
            {
                Assert.That(elevatedSurface.DegreeU, Is.EqualTo(degreeU));
                Assert.That(elevatedSurface.DegreeV, Is.EqualTo(degreeV + 1));
                Assert.That(elevatedSurface.ControlPoints, Is.Not.Empty);
            }
        ...
        [Test]
        public void ElevateDegree_Surface_BothDirections()
        {
            // Arrange: Create a simple NURBS surface
            int degreeU = 2;
            int degreeV = 2;
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    controlPoints[i][j] = new ControlPoint(i, j, Math.Sin(i) * Math.Cos(j) * 2, 1);
                }
            }

            var surface = new NurbsSurface(degreeU, degreeV, kvU, kvV, controlPoints);

            // Act: Elevate degree in both directions
            var elevatedSurface = DegreeOperator.ElevateDegree(surface, 1, 1);
        ...
        [Test]
        public void ElevateDegree_Surface_MultipleTimes()
        {
            // Arrange
            int degreeU = 2;
            int degreeV = 2;
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    controlPoints[i][j] = new ControlPoint(i, j, i + j, 1);
                }
            }

            var surface = new NurbsSurface(degreeU, degreeV, kvU, kvV, controlPoints);

            // Act: Elevate degree multiple times in U direction
            var elevatedSurface = DegreeOperator.ElevateDegree(surface, 2, 0);

            // Assert
            using (Assert.EnterMultipleScope())
            {
                Assert.That(elevatedSurface.DegreeU, Is.EqualTo(degreeU + 2));
                Assert.That(elevatedSurface.DegreeV, Is.EqualTo(degreeV));
            }
        ...
        [Test]
        public void ElevateDegree_Surface_ZeroTimes_ReturnsOriginal()
        {
            // Arrange
            int degreeU = 2;
            int degreeV = 2;
            ...
            // Act: Elevate degree 0 times
            var elevatedSurface = DegreeOperator.ElevateDegree(surface, 0, 0);

            // Assert: Should return same surface
            using (Assert.EnterMultipleScope())
            {
                Assert.That(elevatedSurface.DegreeU, Is.EqualTo(degreeU));
                Assert.That(elevatedSurface.DegreeV, Is.EqualTo(degreeV));
        ...
        [Test]
        public void ElevateDegree_Surface_NullSurface_ThrowsException()
        {
            // Act & Assert
